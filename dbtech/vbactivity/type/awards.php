<?php
class vBActivity_Type_awards extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		$tbl 				= 'dbtech_vbactivity_awardbonus';
		$idfield 			= 'rewardid';
		...
		// Get type id
		$typeid = VBACTIVITY::fetch_type($this->config['typename']);
		...
		{
			// Override points
			VBACTIVITY::$cache['type'][$typeid]['points'] = $result['points'];
	...
	public function check_criteria($conditionid, &$userinfo)
	{
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				...
				FROM " . TABLE_PREFIX . "dbtech_vbactivity_awardbonus
				WHERE userid = " . $userid . "
			");
