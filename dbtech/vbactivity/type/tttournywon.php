<?php
class vBActivity_Type_tttournywon extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		$tbl 				= 'dbt_triad_tournaments';
		$idfield 			= 'tid';
		$datefield 			= 't_ended';
		$hook_query_where 	= " AND t_winner = " . $user['userid'] . "";
		$multiplier 		= 1;
	...
	public function check_criteria($conditionid, &$userinfo)
	{
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				...
				FROM " . TABLE_PREFIX . "dbt_triad_tournaments
				WHERE t_winner = " . $userinfo['userid'] . "
			");
