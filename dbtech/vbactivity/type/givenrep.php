<?php
class vBActivity_Type_givenrep extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		";
		$hook_query_where 	= " AND whoadded = " . $user['userid'];
		$multiplier 		= 1;
	...
	public function check_criteria($conditionid, &$userinfo)
	{
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				SELECT SUM(reputation) AS $typename
				FROM " . TABLE_PREFIX . "reputation
				WHERE whoadded = " . $userid . "
			");
