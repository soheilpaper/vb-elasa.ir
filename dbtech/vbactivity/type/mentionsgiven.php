<?php
class vBActivity_Type_mentionsgiven extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		";		
		$hook_query_where 	= " AND dbtech_usertag_mention.userid = " . $user['userid'];
		$multiplier 		= 1;
	...
	public function check_criteria($conditionid, &$userinfo)
	{
		...
		if (!$userinfo[$typename])
		{
			...
			$additionalinfo = $this->registry->db->query_first_slave("
				...
				FROM " . TABLE_PREFIX . "dbtech_usertag_mention
