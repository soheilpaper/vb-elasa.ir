<?php
class vBActivity_Type_post_own extends vBActivity_Type_Core
{
	/**
	...
	*/	
	public function recalculate_points($user)
	{
		...
		$hook_query_where 	= " AND thread.postuserid = " . $user['userid'] . " AND post.userid = " . $user['userid'] . " AND post.postid != thread.firstpostid";
		$multiplier 		= 1;
	...
	*/	
	public function check_criteria($conditionid, &$userinfo)
	{
		...
		{
			// We need more info
			$userinfo[$typename] = intval(VBACTIVITY::$db->fetchOne('
				SELECT COUNT(*)
				FROM $post AS post
				INNER JOIN $thread AS thread USING(threadid)
