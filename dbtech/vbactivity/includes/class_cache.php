...
class VBACTIVITY_CACHE
{
	...
	* @public	string
	*/	
	public static $prefix 			= 'dbtech_vbactivity_';
	...
	*/	
	protected static $_tables		= array(
		'achievement'		=> '
			LEFT JOIN $dbtech_vbactivity_category AS category ON (category.categoryid = dbtech_vbactivity_achievement.categoryid)
			ORDER BY category.displayorder, dbtech_vbactivity_achievement.displayorder ASC
		',
		'category' 			=> 'ORDER BY `displayorder` ASC',
		'condition' 		=> '',
		'conditionbridge' 	=> '',
		'contest' 			=> 'ORDER BY `end` DESC',
		'contesttype' 		=> 'ORDER BY `title` ASC',
		'medal' 			=> '
			LEFT JOIN $dbtech_vbactivity_category AS category ON (category.categoryid = dbtech_vbactivity_medal.categoryid)
	...
	*/
	public static function init($vbulletin, $cachefields = array(), $exclude = array())
	{
		...
		$cachefields = array_unique($cachefields);
	...
	*/
	public static function build($type)
	{
		...
		if (!is_object(VBACTIVITY::$db))
		{
			// Ensure this doesn't error on upgrades
			return false;
		}

		VBACTIVITY::$db->hideErrors();
		$data = VBACTIVITY::$db->fetchAllKeyed('
			SELECT :dbtype.*
		...
		// Set the data
		VBACTIVITY::$cache[$field_short] = $data;
		
		foreach ((array)VBACTIVITY::$cache[$field_short] as $id => $arr)
		{
			foreach ((array)VBACTIVITY::$unserialize[$field_short] as $key)
			{
				// Do unserialize
				VBACTIVITY::$cache[$field_short][$id][$key] = @unserialize(stripslashes($arr[$key]));
			}
		}
	}
	...
	*/
	public static function read($cacheType, $key)
	{
		...
		// Fetch the vBO data
		$_data = vb_optimise::$cache->get('dbtech.vbactivity.' . $key);
		...
		if (is_array($_data) AND TIMENOW < $_data['time'])
		{
			...
			vb_optimise::stat($i);
			vb_optimise::report('Fetched dbtech.vbactivity.' . $key . ' from cache successfully.');
	...
	*/
	public static function write($data, $cacheType, $key)
	{
		...
		if ($data !== false)
		{
			// Write the vBO data
			$_data = array(
				'time'	=> TIMENOW + (self::$vbulletin->options['vbo_cache_vbactivity' . $cacheType] * 3600),
				'cache'	=> $data,
			);
		}

		// Write the cache
		vb_optimise::$cache->set('dbtech.vbactivity.' . $key, $_data);
		vb_optimise::report('Cached dbtech.vbactivity.' . $key . ' successfully.');	
	...
	*/	
	protected static function _canCache($cacheType = '')
	{
		...
		if (!isset(self::$vbulletin->options['vbo_cache_vbactivity' . $cacheType]))
		{
			// vBO's version is too old
			return false;
		}

		if (!self::$vbulletin->options['vbo_cache_vbactivity' . $cacheType])
		{
