...
// Add to the navbits
$navbits[''] = $pagetitle = $vbphrase['dbtech_vbactivity_medals'];

// draw cp nav bar
VBACTIVITY::setNavClass('medals');
...
$pagenav = construct_page_nav(
	...
	'vbactivity.php?' . $vbulletin->session->vars['sessionurl'] . "do=medals",
	"&amp;perpage=$perpage"
);
...
$medalbits = '';
foreach ($rewards as $day => $medals)
{
	$medalbitbits = '';
	foreach ($medals as $reward)
	{
		// Grab this
		$medal = VBACTIVITY::$cache['medal']["$reward[featureid]"];		
		
		$link = 'member.php?' . $vbulletin->session->vars['sessionurl'] . 'u=' . $reward['userid'] . '&amp;tab=vbactivity';
		
		// Make a readable date
		$medal['dateline'] = vbdate($vbulletin->options['timeformat'], $reward['dateline']);
		...
		$medal['phrase'] = construct_phrase($vbphrase['dbtech_vbactivity_user_gained_medal_x'], 
			'<a href="' . $link . '" target="_blank">' . $reward['musername'] . '</a>',
			($medal['icon'] ? '<img src="images/icons/vbactivity/' . $medal['icon'] . '" alt="' . $medal['title_clean'] . '" /> ' : '') . $medal['title_translated']
		);
		...
		$templaterr = vB_Template::create('dbtech_vbactivity_entrybit');
			$templaterr->register('entry', $medal);
		$medalbitbits .= $templaterr->render();
	}
	...
	$templater = vB_Template::create('dbtech_vbactivity_daybit');
		...
		$templater->register('entrybits', $medalbitbits);
	$medalbits .= $templater->render();
}
...
$templater = vB_Template::create('dbtech_vbactivity_changes');
	...
	$templater->register('changes', 	$medalbits);
