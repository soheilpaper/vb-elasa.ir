...
do
{
	...
	{
		// Grab user info
		$postuserinfo = fetch_userinfo($this->fetch_field('userid'));
	}

	// Type is post
	$points = 'post';
	
	if ($this->info['thread']['postuserid'] == $postuserinfo['userid'])
	{
	...
	// This is the post id
	$idfield = $this->fetch_field('postid');
	
	// Insert points
	VBACTIVITY::insert_points($points, $idfield, $postuserinfo['userid'], 1, TIMENOW, $this->info['thread']['forumid']);
