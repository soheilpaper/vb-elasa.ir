<?php
class LiveWall_ContentType_blogposts extends LiveWall_ContentType_Core
{
	/**
	...
	public function fetchData($lastId = -1, $onlyUser = 0, $limit = -1, $fetchOne = false)
	{
		...
		return LIVEWALL::$db->fetchAll('
			SELECT
				...
				blog.dateline,
				blog.blogid AS contentid,
				\':contentType\' AS contenttypeid,
				favourite.userid IS NOT NULL AS isfavourite				
				:avatarQuery
			FROM (SELECT blogid FROM $blog ' . ($lastId != -1 ? 'WHERE blogid ' . ($fetchOne != false ? '= ' : '> ') . intval($lastId) : '') . ' ORDER BY blogid DESC LIMIT ' . $this->registry->options['dbtech_livewall_maxentries'] . ') AS tmp
			LEFT JOIN $blog AS blog ON(blog.blogid = tmp.blogid)
			LEFT JOIN $blog_text AS blog_text ON(blog_text.blogtextid = blog.firstblogtextid)
			...
			LEFT JOIN $dbtech_livewall_favourite AS favourite ON(favourite.userid = :currentUser AND favourite.contenttypeid = \':contentType\' AND favourite.contentid = blog.blogid)
		...
		', array(
			...
			':fetchOne' 			=> ($fetchOne ? 'LIMIT 1' : 'ORDER BY tmp.blogid DESC'),
	...
	*/	
	public function constructPhrase($info)
	{
		...
		return construct_phrase($vbphrase['dbtech_livewall_x_posted_blog_y'], 
			...
			'<a href="entry.php?' . $this->registry->session->vars['sessionurl'] . 'b=' . $info['blogid'] . '">' . $info['blogtitle'] . '</a>'
		);
	}	
}
?>