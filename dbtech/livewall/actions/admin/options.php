...
if ($_REQUEST['action'] == 'options' OR empty($_REQUEST['action']))
{
	...
	if ($_REQUEST['do'] == 'options' AND !empty($vbulletin->GPC['varname']))
	{
		...
		else if ($group = $db->query_first("SELECT varname, grouptitle FROM " . TABLE_PREFIX . "setting WHERE varname = '" . $db->escape_string($vbulletin->GPC['varname']) . "'"))
		{
			...
			exec_header_redirect('livewall.php?' . $vbulletin->session->vars['sessionurl_js'] . "do=options&dogroup=$group[grouptitle]#$group[varname]");
		}
	}
	...
	print_cp_header($vbphrase['dbtech_livewall_settings']);
	...
	$settings = $db->query_read("
		...
		WHERE settinggroup.product LIKE 'dbtech_livewall%'
	...
	print_form_header('livewall', 'options', 0, 1, 'groupForm', '90%', '', 1, 'get');
	...
	else // show the small list with selected setting group(s) options
	{
		...
		print_form_header('livewall', 'options', false, true, 'optionsform', '90%', '', true, 'post" onsubmit="return count_errors()');
...
if ($_REQUEST['action'] == 'dooptions')
{
	...
	if (!empty($vbulletin->GPC['setting']))
	{
		...
		define('CP_REDIRECT', 'livewall.php?do=options&amp;dogroup=' . $vbulletin->GPC['dogroup'] . '&amp;advanced=' . $vbulletin->GPC['advanced']);
