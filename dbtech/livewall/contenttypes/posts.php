<?php
class LiveWall_ContentType_posts extends LiveWall_ContentType_Core
{
	/*DBTECH_PRO_START*/
	/**
	...
	public function fetchData($lastId = -1, $onlyUser = 0, $limit = -1, $fetchOne = false)
	{
		...
		return LIVEWALL::$db->fetchAll('
			SELECT
				...
				post.postid AS contentid,
				\':contentType\' AS contenttypeid,
				favourite.userid IS NOT NULL AS isfavourite				
				:avatarQuery
			FROM (SELECT postid FROM $post ' . ($lastId != -1 ? 'WHERE postid ' . ($fetchOne != false ? '= ' : '> ') . intval($lastId) : '') . ' ORDER BY postid DESC LIMIT ' . $this->registry->options['dbtech_livewall_maxentries'] . ') AS tmp
			...
			LEFT JOIN $dbtech_livewall_favourite AS favourite ON(favourite.userid = :currentUser AND favourite.contenttypeid = \':contentType\' AND favourite.contentid = post.postid)
		...
		', array(
			...
			':fetchOne' 			=> ($fetchOne ? 'LIMIT 1' : 'ORDER BY tmp.postid DESC'),
	...
	*/	
	public function constructPhrase($info)
	{
		...
		return construct_phrase($vbphrase['dbtech_livewall_x_posted_in_thread_y'], 
