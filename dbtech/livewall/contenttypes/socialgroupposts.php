<?php
class LiveWall_ContentType_socialgroupposts extends LiveWall_ContentType_Core
{
	/**
	...
	public function fetchData($lastId = -1, $onlyUser = 0, $limit = -1, $fetchOne = false)
	{
		...
		return LIVEWALL::$db->fetchAll('
			SELECT
				groupmessage.*,
				user.*,
				firstpost.title AS threadtitle,
				socialgroup.name AS socialgroupname,
				socialgroup.groupid,
				groupmessage.dateline,
				groupmessage.gmid AS contentid,
				...
				:avatarQuery
			FROM (SELECT gmid FROM $groupmessage ' . ($lastId != -1 ? 'WHERE gmid ' . ($fetchOne != false ? '= ' : '> ') . intval($lastId) : '') . ' ORDER BY gmid DESC LIMIT ' . $this->registry->options['dbtech_livewall_maxentries'] . ') AS tmp
			LEFT JOIN $groupmessage AS groupmessage ON(groupmessage.gmid = tmp.gmid)
			LEFT JOIN $discussion AS discussion ON(discussion.discussionid = groupmessage.discussionid)
			LEFT JOIN $groupmessage AS firstpost ON(firstpost.gmid = discussion.firstpostid)
			...
			LEFT JOIN $dbtech_livewall_favourite AS favourite ON(favourite.userid = :currentUser AND favourite.contenttypeid = \':contentType\' AND favourite.contentid = groupmessage.gmid)
			...
			WHERE groupmessage.state = \':state\'
				AND groupmessage.gmid != discussion.firstpostid
		...
		', array(
			...
			':fetchOne' 			=> ($fetchOne ? 'LIMIT 1' : 'ORDER BY tmp.gmid DESC'),
	...
	*/	
	public function constructPhrase($info)
	{
		...
		return construct_phrase($vbphrase['dbtech_livewall_x_posted_in_social_group_y_thread_z'], 
