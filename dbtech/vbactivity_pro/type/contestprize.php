<?php
class vBActivity_Type_contestprize extends vBActivity_Type_Core
{
	/**
	...
	public function recalculate_points($user)
	{
		...
		foreach ((array)VBACTIVITY::$cache['contest'] as $contestid => $contest)
		{
			...
			foreach ((array)$contest['winners'] as $place => $winner)
			{
				...
				// Insert the points
				VBACTIVITY::insert_points($this->config['typename'], $contestid, $user['userid'], $contest['prizes2'][$place], $contest['enddate']);
