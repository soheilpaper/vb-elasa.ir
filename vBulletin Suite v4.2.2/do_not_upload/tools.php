...
if (empty($_REQUEST['do']))
{
	...
	$mobilefile = 'vbulletin-mobile-style.xml';
	if ($fp = @fopen('./install/' . $mobilefile, 'rb'))
	{
		$data = fread($fp, 256);

		if (preg_match('#vbversion="(.*?)"#', $data, $matches))
		{
			$mobilestyle_xml = $matches[1];
		}
		else
		{
			$mobilestyle_xml = 'Unknown';
		}
		fclose($fp);
	}
	else
	...
	print_label_row(construct_link_code('Mobile Style', THIS_SCRIPT . '?do=xml&amp;type=mobilestyle'), "This will take the latest mobile style from ./install/{$mobilefile}<dfn>Version: <b>$mobilestyle_xml</b></dfn>");
	...
	print_column_style_code(array('width:30%'));
	$vbulletin->options['cookiedomain'] = $vbulletin->options['cookiedomain'] == '' ? ' ( blank ) ' : '<b>' . htmlspecialchars_uni($vbulletin->options['cookiedomain']) . '</b>';
	$vbulletin->options['cookiepath'] = $vbulletin->options['cookiepath'] == '' ? ' ( blank ) ' : '<b>' . htmlspecialchars_uni($vbulletin->options['cookiepath']) . '</b>';
...
else if ($_REQUEST['do'] == 'xml')
{
	switch ($vbulletin->GPC['type'])
	{
		...
		case 'style':
			...
			if ($vbulletin->GPC['type'] == 'style')
			{
				$file = 'vbulletin-style.xml';
				$masterstyleid = -1;
				$mastertype = 'standard';
			}
			else
			{
				$file = 'vbulletin-mobile-style.xml';
				$masterstyleid = -2;
				$mastertype = 'mobile';
			}

			if (!($xml = file_read("./install/{$file}")))
			{
				echo "<p>Uh oh, ./install/{$file} doesn't appear to exist! Upload it and refresh the page.</p>";
				break;
			}

			echo "<p>Importing {$file}</p>";

			$startat = $vbulletin->input->clean_gpc('r', 'startat', TYPE_UINT);

			$vbphrase['go_back'] = 'Go Back';
			$vbphrase['template_group_x'] = 'Template Group: %1$s';

			$perpage = 10;
			$imported = xml_import_style($xml, $masterstyleid, $masterstyleid, '', false, 1, false, $startat, $perpage, 0, $file);
			if (!$imported['done'])
			{
				//build the next page url;
				$startat = $startat + $perpage;
				$url = "tools.php?do=xml&type={$vbulletin->GPC['type']}&startat=" . $startat;
			...
			build_all_styles(0, 1, '', false, $mastertype);

			print_cp_redirect('tools.php?do=templatemerge&masterstyleid=' . $masterstyleid);
		break;
		case 'settings':
...
else if ($_REQUEST['do'] == 'templatemerge')
{
	$vbulletin->input->clean_array_gpc('r', array(
		'startat'       => TYPE_UINT,
		'masterstyleid' => TYPE_INT
	));

	if ($vbulletin->GPC['masterstyleid'] != -2)
	{
		$vbulletin->GPC['masterstyleid'] = -1;
	}
	...
	$merge->time_limit = 5;
	$completed = $merge->merge_templates($merge_data, $output, $vbulletin->GPC['masterstyleid']);
	...
	if ($completed)
	{
		...
		build_all_styles(0, 0, "", false, $vbulletin->GPC['masterstyleid']);
	...
	else
	{
		...
		print_cp_redirect(
			'tools.php?do=templatemerge&startat=' . ($merge_data->start_offset + $merge->fetch_processed_count() . '&masterstyleid=' . $vbulletin->GPC['masterstyleid'])
		);
	}
}
