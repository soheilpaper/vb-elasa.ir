...
function add_thanks($postinfo)
{
	...
	//version 7.85 edit by achoo254
	if ($vbulletin->options['post_thanks_credits'])
	{
		if($vbulletin->options['post_thanks_user_thanked']==1)
		{
			$vbulletin->db->query_first_slave("
				SELECT " . $vbulletin->options['post_thanks_credits_thank'] . "
 				FROM " . TABLE_PREFIX . "user
				WHERE  '".$postinfo['postid']."' = userid
				");
			$set_user_got .= ", " . $vbulletin->options['post_thanks_credits_thank'] . " = " . $vbulletin->options['post_thanks_credits'] . " + " . $vbulletin->options['post_thanks_credits_thank'] . "";
		}
		if($vbulletin->options['post_thanks_user_thank']==1)
		{
			$vbulletin->db->query_first_slave("
				SELECT " . $vbulletin->options['post_thanks_credits'] . "
 				FROM " . TABLE_PREFIX . "user
				WHERE  '".$vbulletin->userinfo['userid']."' = userid
				");
			$set_user_gave .= ", " . $vbulletin->options['post_thanks_credits_thank'] . " = " . $vbulletin->options['post_thanks_credits'] . " + " . $vbulletin->options['post_thanks_credits_thank'] . "";
		}
	}
	//ket thuc ham
...
function delete_thanks($postinfo, $userid, $remove_users_thanks_count = true)
{
	...
	//version 7.85 edit by achoo254
	if ($vbulletin->options['post_thanks_credits'] && ($remove_users_thanks_count == true || $vbulletin->options['post_thanks_delete_remove_thanks_credits']))
	{
		if($vbulletin->options['post_thanks_user_thank'])
		{
			$set_user_gave .= ", " . $vbulletin->options['post_thanks_credits_thank'] . " = " . $vbulletin->options['post_thanks_credits_thank'] . " - " . $vbulletin->options['post_thanks_credits'] . "";
		}
		if ($vbulletin->options['post_thanks_user_thanked'])
		{
			$set_user_got .= ", " . $vbulletin->options['post_thanks_credits_thank'] . " = " . $vbulletin->options['post_thanks_credits_thank'] . " - " . $vbulletin->options['post_thanks_credits'] . "";
		}
	}
	//ket thuc ham
